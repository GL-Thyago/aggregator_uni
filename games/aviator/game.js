(function () {
  ...
  function drawPlane(ctx, x, y, crashed) {
    ...
    ctx.translate(x, y);
    ctx.rotate(-0.35);
    ctx.fillStyle = crashed ? "#888" : "#e50539";
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(-18, 6);
    ctx.lineTo(-14, 0);
    ...
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = crashed ? "#666" : "#ff4d6d";
    ctx.beginPath();
  ...
  function drawFrame(progress, mult, crashed) {
    ...
    for (var i = 0; i < 24; i++) {
      ...
      grd.addColorStop(0, "rgba(229, 5, 57, 0.35)");
    ...
    for (var t = 0; t <= maxT; t += 0.02) {
      var px = cx + Math.pow(t, 0.85) * (w - cx - 30);
      var py = cy - Math.pow(t, 1.45) * (cy - 30);
    ...
    if (points.length > 1) {
      ...
      ctx.lineTo(cx, cy);
      ctx.closePath();
      ctx.fillStyle = crashed ? "rgba(229,5,57,0.12)" : "rgba(229,5,57,0.18)";
      ctx.fill();
    }
