(function () {
  ...
  function drawPlane(ctx, x, y, crashed) {
    ...
    ctx.translate(x, y);
    ctx.rotate(-0.2);
    ctx.fillStyle = crashed ? "#666" : "#c084fc";
    ctx.beginPath();
    ctx.arc(0, 0, 14, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = crashed ? "#444" : "#fff";
    ctx.beginPath();
    ctx.arc(4, -3, 5, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = crashed ? "#555" : "#a855f7";
  ...
  function drawFrame(progress, mult, crashed) {
    ...
    if (points.length > 1) {
      ...
      ctx.lineTo(cx, cy);
      ctx.closePath();
      ctx.fillStyle = crashed ? "rgba(168,85,247,0.12)" : "rgba(168,85,247,0.18)";
      ctx.fill();
    }
